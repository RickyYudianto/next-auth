...
exports.generateMigrationLetObject = void 0;
...
var generateMigrationLetObject = function (migrations) {
    var indexedMigrations = indexMigrations(migrations);
    var dependencyIndex = findDependencies(migrations);
    ...
    var dependenciesArray = indexToDependenciesArray(dependencyIndex);
    // Todo, add names of previous migrations for extra verification
    // to make sure no unexisting resource is deleted/updated.
    var orderedExpressions = orderAccordingToDependencies(dependenciesArray, indexedMigrations);
    ...
    var letBindingObject = generateLetBindingObject(orderedExpressions, indexedMigrations, dependencyIndex);
    return letBindingObject;
};
exports.generateMigrationLetObject = generateMigrationLetObject;
var reOrderDeleted = function (orderedExpressions) {
    ...
    var deleted = orderedExpressions.filter(function (e) { return e.statement === expressions_1.StatementType.Delete; });
    var rest = orderedExpressions.filter(function (e) { return e.statement !== expressions_1.StatementType.Delete; });
    var deletedIndexes = deleted.filter(function (e) { return e.type === resource_types_1.ResourceTypes.Index; });
    var deletedRest = deleted.filter(function (e) { return e.type !== resource_types_1.ResourceTypes.Index; });
...
var replaceWithStubs = function (dependenciesArray, indexedMigrations, orderedExpressions) {
    ...
    while (dependenciesArray.length > 0) {
        ...
        if (expr.statement === expressions_1.StatementType.Create) {
            orderedExpressions.push((0, transform_1.createStub)(expr));
            stubbed.push((0, transform_1.transformCreateToUpdate)(expr));
        }
        else {
            others.push(expr);
        }
...
var generateLetBindingObject = function (orderedExpressions, indexedMigrations, dependencyIndex) {
    ...
    orderedExpressions.forEach(function (e, varIndex) {
        var indexableName = (0, unique_naming_1.toIndexableName)(e);
        ...
        dependencies.forEach(function (dep) {
            ...
            if (depExpr.statement === expressions_1.StatementType.Create) {
                var depIndexableName = (0, unique_naming_1.toIndexableName)(depExpr);
                ...
                // instead directly.
                e.fql = replaceAll(e.fql, "".concat(depExpr.type, "(\"").concat(depExpr.name, "\")"), "Var(\"".concat(nameToVar[depIndexableName], "\")"));
            }
        });
        if (nameToVar[indexableName] && e.statement === expressions_1.StatementType.Update) {
            e.fql = replaceAll(e.fql, "".concat(e.type, "(\"").concat(e.name, "\")"), "Var(\"".concat(nameToVar[indexableName], "\")"));
        }
        ...
        if (!nameToVar[indexableName]) {
            nameToVar[indexableName] = "var".concat(varIndex);
        }
        // Bind the variable to the code.
        obj["var".concat(varIndex)] = (0, eval_1.evalFQLCode)("Select(['ref'], ".concat(e.fql, ")"));
    });
    return obj;
};
...
var indexMigrations = function (flattened) {
    ...
    flattened.forEach(function (expr) {
        var indexableName = (0, unique_naming_1.toIndexableName)(expr);
...
var indexReferencedDependencies = function (flattened, jsonPatterns) {
    ...
    flattened.forEach(function (expr) {
        var indexableName = (0, unique_naming_1.toIndexableName)(expr);
        var found = (0, json_1.findPatterns)(expr.fqlExpr, jsonPatterns);
...
var toPattern = function (mig, type) {
    ...
    switch (type) {
        case resource_types_1.ResourceTypes.Function:
            return { pattern: { raw: { function: mig.name } }, indexName: (0, unique_naming_1.toIndexableName)(mig) };
        case resource_types_1.ResourceTypes.Role:
            return { pattern: { raw: { role: mig.name } }, indexName: (0, unique_naming_1.toIndexableName)(mig) };
        case resource_types_1.ResourceTypes.Collection:
            return { pattern: { raw: { collection: mig.name } }, indexName: (0, unique_naming_1.toIndexableName)(mig) };
        case resource_types_1.ResourceTypes.Index:
            return { pattern: { raw: { index: mig.name } }, indexName: (0, unique_naming_1.toIndexableName)(mig) };
        case resource_types_1.ResourceTypes.AccessProvider:
            return { pattern: { raw: { access_provider: mig.name } }, indexName: (0, unique_naming_1.toIndexableName)(mig) };
        case resource_types_1.ResourceTypes.Database:
            return { pattern: { raw: { database: mig.name } }, indexName: (0, unique_naming_1.toIndexableName)(mig) };
        default:
            throw new Error("unknown type in toPattern ".concat(type));
    }
};
var indexToDependenciesArray = function (dependencyIndex) {
    return Object.keys(dependencyIndex)
        .map(function (indexedName) {
