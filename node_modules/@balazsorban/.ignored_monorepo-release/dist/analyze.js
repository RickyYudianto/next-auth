...
async function getPackages(rootDir, workspaceDirs) {
    const packages = {};
...
export async function analyze(config) {
    const { BREAKING_COMMIT_MSG, RELEASE_COMMIT_MSG, RELEASE_COMMIT_TYPES } = config;
    ...
    const commitsSinceLatestTag = await new Promise((resolve, reject) => {
        const stream = gitLog.parse({ _: range });
        streamToArray(stream, (err, arr) => {
            if (err)
                return reject(err);
            Promise.all(arr.map(async (d) => {
                // @ts-ignore
    ...
    debug("Analyzing the following commits:", commitsSinceLatestTag.map((c) => `  ${c.subject}`).join("\n"));
    ...
    function getChangedFiles(commitSha) {
        return execSync(`git diff-tree --no-commit-id --name-only -r ${commitSha}`, { stdio: "pipe" })
            .toString()
            .trim()
            .split("\n");
    }
    const packageCommits = commitsSinceLatestTag.filter(({ commit }) => {
        ...
        return packageList.some((packageFolder) => changedFiles.some((changedFile) => changedFile.startsWith(packageFolder)));
    });
    console.log(packageCommits.length, "commits touched package code");
    console.log();
    console.log("Identifying packages that need a new release...");
    const packagesNeedRelease = [];
    const grouppedPackages = packageCommits.reduce((acc, commit) => {
        ...
        for (const [pkg, src] of Object.entries(packages)) {
            if (changedFilesInCommit.some((changedFile) => changedFile.startsWith(src))) {
                if (!(pkg in acc)) {
    ...
    if (packagesNeedRelease.length) {
        console.log(packagesNeedRelease.length, `new release(s) needed: ${packagesNeedRelease.join(", ")}`);
    }
    else {
    ...
    for await (const pkgName of packagesNeedRelease) {
        ...
        const releaseType = commits.breaking.length
            ? "major" // 1.x.x
