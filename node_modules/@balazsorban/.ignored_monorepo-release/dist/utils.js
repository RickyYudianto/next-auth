...
async function read(directory) {
    const content = await fs.readFile(path.join(process.cwd(), directory, "package.json"), "utf8");
    return JSON.parse(content);
}
async function update(directory, data) {
    const original = await pkgJson.read(directory);
    const content = JSON.stringify({ ...original, ...data }, null, 2);
    await fs.writeFile(path.join(process.cwd(), directory, "package.json"), content, "utf8");
}
