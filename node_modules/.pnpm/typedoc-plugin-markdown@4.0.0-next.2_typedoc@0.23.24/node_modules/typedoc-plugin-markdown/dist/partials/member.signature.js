...
function signatureMember(context, signature, parentHeadingLevel) {
    ...
    const headingLevel = (parentHeadingLevel
        ? parentHeadingLevel
        : (0, helpers_1.getReflectionHeadingLevel)(signature.parent)) + 1;
    md.push((0, els_1.heading)(headingLevel, 'Signature'));
    md.push((0, els_1.codeBlock)(context.partials.signatureTitle(signature)));
    if (signature.comment) {
        md.push(context.partials.comment(signature.comment, headingLevel));
    }
    if (parentHeadingLevel) {
        md.push((0, els_1.indentBlock)(signatureBody(context, signature, parentHeadingLevel)));
    }
    else {
...
function signatureBody(context, signature, parentHeadingLevel) {
    var _a, _b, _c, _d;
    const md = [];
    const headingLevel = (parentHeadingLevel
        ? parentHeadingLevel
