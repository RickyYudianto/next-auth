...
async function sortByDependency(pkgs) {
    const pkgsWithDeps = new Map();
...
export async function publish(packages, options) {
    ...
    for await (const pkg of packages) {
        if (dryRun) {
            console.log(`Dry run, \`npm publish\` would have released package \`${pkg.name}\` with version "${pkg.newVersion}".`);
        }
        else {
            console.log(`Writing version "${pkg.newVersion}" to package.json for package \`${pkg.name}\``);
        ...
        if (dryRun) {
            ...
            execSync("echo '//registry.npmjs.org/:_authToken=${NPM_TOKEN}' > .npmrc", { cwd: pkg.path });
        }
...
function createChangelog(pkg) {
    const { commits: { features, breaking, bugfixes, other }, } = pkg;
...
function listGroup(heading, commits) {
    if (!commits.length)
        return "";
